...
function _add_obstacle_bounce_back!(active_cells::Matrix{Bool}, 
  ...
  push!(bcs!, sim -> west_bounce_back!(sim.lat, i_max, j_min, j_max));
end
...
function _add_obstacle_reflector!(active_cells::Matrix{Bool}, 
                                  ...
                                  j_max::Int)
  active_cells[i_min+1:i_max-1, j_min+1:j_max-1] = false;
  push!(bcs!, sim -> north_reflect!(sim.lat, i_min, i_max, j_min));
  push!(bcs!, sim -> south_reflect!(sim.lat, i_min, i_max, j_max));
  push!(bcs!, sim -> east_reflect!(sim.lat, i_min, j_min, j_max));
  push!(bcs!, sim -> west_reflect!(sim.lat, i_max, j_min, j_max));
end
